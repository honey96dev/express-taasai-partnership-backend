...
const signInProc = async (req, res, next) => {
  ...
  let sql = sprintf("SELECT * FROM %s WHERE email = $1;", dbTblName.merchants);
  try {
    ...
    res.status(200).send({
      ...
      data: {
        user: {...user, user_type: consts.auth.USER_TYPE.MERCHANT},
        token,
      },
    });
  } catch (err) {
...
const signUpProc = async (req, res, next) => {
  ...
  let sql = sprintf("SELECT email FROM %s WHERE email = $1;", dbTblName.merchants);
  try {
    let {rows, rowCount} = await db.query(sql, [email]);
    if (rowCount > 0) {
      res.status(200).send({
    ...
    const newRows = [
      [timestamp, timestamp, null, "", "", 0, 0, "", "", "", true, "", 0, 0, 0, "", email, hash],
    ];
    sql = pgformat("INSERT INTO %s(created_at, updated_at, deleted_at, name, address, cashback, pending, contact, open_time, close_time, is_enabled, offer_name, minimum_spend, redemption, revenue, logo, email, hash) VALUES %L RETURNING id;", dbTblName.merchants, newRows);
    await db.query(sql);

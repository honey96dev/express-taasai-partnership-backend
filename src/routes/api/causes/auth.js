...
const signInProc = async (req, res, next) => {
  ...
  let sql = sprintf("SELECT * FROM %s WHERE email = $1;", dbTblName.causes);
  try {
    let {rows, rowCount} = await db.query(sql, [email]);
    if (rowCount === 0) {
      res.status(200).send({
        result: langs.error,
    ...
    if (!!user.deleted_at && user.deleted_at > 0) {
      res.status(200).send({
        result: langs.error,
    ...
    res.status(200).send({
      ...
      data: {
        user: {...user, user_type: consts.auth.USER_TYPE.CAUSE},
        token,
      },
    });
  } catch (err) {
...
const signUpProc = async (req, res, next) => {
  ...
  let sql = sprintf("SELECT email FROM %s WHERE email = $1;", dbTblName.causes);
  try {
    let {rows, rowCount} = await db.query(sql, [email]);
    if (rowCount > 0) {
      res.status(200).send({
    ...
    const newRows = [
      [timestamp, timestamp, null, "", "", email, "", "", "", "", "", true, "", 0, "", "", "", "", "", hash],
    ];
    sql = pgformat("INSERT INTO %s(created_at, updated_at, deleted_at, name, address, email, dial_code, phone, logo, manager_name1, manager_name2, is_enabled, about, balance, manager_title, join_reason, social_media, social_frequency, major_event, hash) VALUES %L RETURNING id;", dbTblName.causes, newRows);
    await db.query(sql);
